...
let Repository = class Repository {
    ...
    findBy(key, value) {
        let find = {};
        find[key] = value;
        return this.toModel(this.query.find(find).value());
    }
    filter(filter = {}) {
        return this.toModel(this.query.filter(filter).value());
    }
    count() {
        return parseInt(this.query.size().value());
    }
    toModel(val) {
        if (util_1.kindOf(val) === 'array') {
            if (val.length === 0)
                return [];
            return val.map((data) => this.model(data));
        }
